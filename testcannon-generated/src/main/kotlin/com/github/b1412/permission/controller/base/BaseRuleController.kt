...
abstract class BaseRuleController(

) : BaseController<Rule, Long>() {

    @PostMapping("graph")
    override fun graph(@RequestBody body: String, pageable: Pageable, request: HttpServletRequest): ResponseEntity<Page<Rule>> {
    ...
    @GetMapping
    override fun page(pageable: Pageable, request: HttpServletRequest): ResponseEntity<Page<Rule>> {
        return super.page(pageable, request)
    }

    @GetMapping("{id}")
    override fun findOne(@PathVariable id: Long, request: HttpServletRequest): ResponseEntity<Rule> {
