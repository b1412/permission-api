...
abstract class BaseBranchController(
    ...
    @PostMapping("graph")
    override fun graph(@RequestBody body: String, pageable: Pageable, request: HttpServletRequest): ResponseEntity<Page<Branch>> {
    ...
    @GetMapping
    override fun page(pageable: Pageable, request: HttpServletRequest): ResponseEntity<Page<Branch>> {
        return super.page(pageable, request)
    }

    @GetMapping("{id}")
    override fun findOne(@PathVariable id: Long, request: HttpServletRequest): ResponseEntity<Branch> {
