...
import com.github.b1412.permission.controller.base.BaseBlogController
...
abstract class BaseBlogController(

) : BaseController<Blog, Long>() {

    @PostMapping("graph")
    override fun graph(@RequestBody body: String, pageable: Pageable, request: HttpServletRequest): ResponseEntity<Page<Blog>> {
    ...
    @GetMapping
    override fun page(pageable: Pageable, request: HttpServletRequest): ResponseEntity<Page<Blog>> {
        return super.page(pageable, request)
    }

    @GetMapping("{id}")
    override fun findOne(@PathVariable id: Long, request: HttpServletRequest): ResponseEntity<Blog> {
        return super.findOne(id, request)
    }

    @Transactional
    @PostMapping
    override fun saveOne(@Validated @RequestBody input: Blog, request: HttpServletRequest): ResponseEntity<*> {
        return super.saveOne(input, request)
    }

    @Transactional
    @PutMapping("{id}")
    override fun updateOne(@PathVariable id: Long, @Validated @RequestBody input: Blog, request: HttpServletRequest): ResponseEntity<*> {
