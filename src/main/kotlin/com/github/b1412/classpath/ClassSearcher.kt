...
class ClassSearcher(private val target: Class<*>) {
    ...
    val log = LoggerFactory.getLogger(com.github.b1412.classpath.ClassSearcher::class.java)!!
    private var classpath = com.github.b1412.classpath.PathKit.Companion.getRootClassPath()
    ...
    fun <T> search(): List<Class<out T>> {
        ...
        if (scanPackages.isEmpty()) {
            classFileList = com.github.b1412.classpath.ClassSearcher.Companion.findFiles(classpath, "*.class")
        } else {
            for (scanPackage in scanPackages) {
                classFileList = com.github.b1412.classpath.ClassSearcher.Companion.findFiles(classpath + File.separator + scanPackage.replace("\\.".toRegex(), "\\" + File.separator), "*.class")
            }
        }
        classFileList.addAll(findjarFiles(libDir))
        return com.github.b1412.classpath.ClassSearcher.Companion.extraction<T>(target as Class<T>, classFileList)
    }
    ...
    fun injars(jars: List<String>?): com.github.b1412.classpath.ClassSearcher {
        if (jars != null) {
    ...
    fun inJars(vararg jars: String): com.github.b1412.classpath.ClassSearcher {
        if (jars != null) {
            for (jar in jars) {
                includeJars.add(jar)
            }
    ...
    fun includeAllJarsInLib(includeAllJarsInLib: Boolean): com.github.b1412.classpath.ClassSearcher {
        this.includeAllJarsInLib = includeAllJarsInLib
        return this
    }

    fun classpath(classpath: String): com.github.b1412.classpath.ClassSearcher {
        this.classpath = classpath
        return this
    }

    fun libDir(libDir: String): com.github.b1412.classpath.ClassSearcher {
        this.libDir = libDir
        return this
    }

    fun scanPackages(scanPaths: List<String>?): com.github.b1412.classpath.ClassSearcher {
        if (scanPaths != null) {
    ...
    companion object {
        ...
        fun of(target: Class<*>): com.github.b1412.classpath.ClassSearcher {
            return com.github.b1412.classpath.ClassSearcher(target)
        }
            ...
            if (!baseDir.exists() || !baseDir.isDirectory) {
               // log.error("search error：" + baseDirName + "is not a dir！")
                ...
                for (i in files!!.indices) {
                    ...
                    if (file.isDirectory) {
                        classFiles.addAll(com.github.b1412.classpath.ClassSearcher.Companion.findFiles(baseDirName + File.separator + files[i], targetFileName))
                    } else {
                        if (com.github.b1412.classpath.ClassSearcher.Companion.wildcardMatch(targetFileName, file.name)) {
                            val fileName = file.absolutePath
                            val open = com.github.b1412.classpath.PathKit.Companion.getRootClassPath() + File.separator
                            val close = ".class"
            ...
            for (patternIndex in 0 until patternLength) {
                ...
                if (ch == '*') {
                    ...
                    while (strIndex < strLength) {
                        if (com.github.b1412.classpath.ClassSearcher.Companion.wildcardMatch(pattern.substring(patternIndex + 1), fileName.substring(strIndex))) {
                            return true
                        }
                        strIndex++
                    }
